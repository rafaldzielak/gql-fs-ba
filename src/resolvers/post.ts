...
export class PostResolver {
  ...
  async voteStatus(@Root() post: Post, @Ctx() { updootLoader, req }: MyContext) {
    ...
    const updoot = await updootLoader.load({ postId: post.id, userId: (req.session as any).userId });
  ...
  async updatePost(
    ...
    const result = await dataSource
      ...
      .update(Post)
      .set({ title, text })
      .where('id = :id and "creatorId" = :creatorId', { id, creatorId: (req.session as any).userId })
      .returning("*")
      .execute();
    console.log(result);
    return result.raw[0];
